...
import { createRelayEnvironment } from "./network/relay-environment";
...
export function Layout({ children }: { children: React.ReactNode }) {
	return (
		<html lang="en" className="dark">
			...
			<body style={{ backgroundColor: "#121113" }}>
				{children}
				<ScrollRestoration />
				<Scripts />
			</body>
...
export default function App() {
	...
	const relayEnvironmentRef = useRef(
		createRelayEnvironment(handleError, handleLogout),
	);

	useEffect(() => {
		if (!isLoggedIn) {
			commitLocalUpdate(relayEnvironmentRef.current, (relayStore) => {
				relayStore.invalidateStore();
			});
		}
	}, [isLoggedIn]);
